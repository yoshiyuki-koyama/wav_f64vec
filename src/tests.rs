...
mod tests {
    ...
    fn read_write_convert_test() {
        {
            ...
            check_contents(&path_buf, &vec_of_fmt_chunk, &vec_of_data_chunk, &vec_of_file, &expected_channel_vec);
...

        {
            ...
            let  (vec_of_fmt_chunk, vec_of_data_chunk, vec_of_file) = create_u8_vecs(
                ...
                &[0x2C, 0x00, 0x00, 0x00],
                &[
                    0x00,0x00,0x00,0x00,0xFF,0x3F,0x01,0xC0,0xFF,0x7F,0x01,0x80,0xFF,0x7F,0x01,0x80,
                    0xFF,0x3F,0x01,0xC0,0x00,0x00,0x00,0x00,0x01,0xC0,0xFF,0x3F,0x01,0x80,0xFF,0x7F,
                    0x01,0x80,0xFF,0x7F,0x01,0xC0,0xFF,0x3F,0x00,0x00,0x00,0x00],
            );
            check_contents(&path_buf, &vec_of_fmt_chunk, &vec_of_data_chunk, &vec_of_file, &expected_channel_vec);
            remove_file(&path_buf).unwrap();
        }

        {
            ...
            let  (vec_of_fmt_chunk, vec_of_data_chunk, vec_of_file) = create_u8_vecs(
                ...
                &[0x21, 0x00, 0x00, 0x00],
                &[
                    0x00, 0x00, 0x00, 0xFF, 0xFF, 0x3F, 0xFF, 0xFF, 0x7F, 0xFF, 0xFF, 0x7F, 0xFF, 0xFF, 0x3F, 0x00,
                    0x00, 0x00, 0x01, 0x00, 0xC0, 0x01, 0x00, 0x80, 0x01, 0x00, 0x80, 0x01, 0x00, 0xC0, 0x00, 0x00,
                    0x00],
            );
            check_contents(&path_buf, &vec_of_fmt_chunk, &vec_of_data_chunk, &vec_of_file, &expected_channel_vec);
            remove_file(&path_buf).unwrap();
        ...
        
        {
            ...
            let  (vec_of_fmt_chunk, vec_of_data_chunk, vec_of_file) = create_u8_vecs(
                ...
                &[0x2C, 0x00, 0x00, 0x00],
                &[
                    0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0x3F, 0xFF, 0xFF, 0xFF, 0x7F, 0xFF, 0xFF, 0xFF, 0x7F,
                    0xFF, 0xFF, 0xFF, 0x3F, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0xC0, 0x01, 0x00, 0x00, 0x80,
                    0x01, 0x00, 0x00, 0x80, 0x01, 0x00, 0x00, 0xC0, 0x00, 0x00, 0x00, 0x00],
            );
            check_contents(&path_buf, &vec_of_fmt_chunk, &vec_of_data_chunk, &vec_of_file, &expected_channel_vec);
            remove_file(&path_buf).unwrap();
        }

        {
            ...
            let written_channel_vec: Vec<Vec<f64>> = vec![
                vec![0.00,  half, 1.00, f32::MAX as f64 + 1.0, half, 0.00, -half, -1.00, f32::MIN as f64 - 1.0, -half, 0.00]];
            let expected_channel_vec: Vec<Vec<f64>> = vec![
                vec![0.00,  half, 1.00, f32::MAX as f64, half, 0.00, -half, -1.00, f32::MIN as f64, -half, 0.00]];
            ...
            let  (vec_of_fmt_chunk, vec_of_data_chunk, vec_of_file) = create_u8_vecs(
                ...
                &[0x2C, 0x00, 0x00, 0x00],
                &[
                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3F, 0x00, 0x00, 0x80, 0x3F, 0xFF, 0xFF, 0x7F, 0x7F,
                    0x00, 0x00, 0x00, 0x3F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xBF, 0x00, 0x00, 0x80, 0xBF, 
                    0xFF, 0xFF, 0x7F, 0xFF, 0x00, 0x00, 0x00, 0xBF, 0x00, 0x00, 0x00, 0x00],
            );
            check_contents(&path_buf, &vec_of_fmt_chunk, &vec_of_data_chunk, &vec_of_file, &expected_channel_vec);
            remove_file(&path_buf).unwrap();
        }
    ...
    fn vec_order_test() {
        ...
        for channel_idx in 0..channel_data_vec.len() {
            for data_idx in 0..data_channel_vec.len() {
                assert_eq!(channel_data_vec[channel_idx][data_idx], data_channel_vec[data_idx][channel_idx]);
            }
        }
    ...
    fn create_test_file(id: usize, channel: usize, sampling_rate: usize, bits: usize, channel_vec:&Vec<Vec<f64>>) -> PathBuf {
        ...
        let path_string = format!("./test_id{}_{}ch_{}hz_{}bits.wav", wave_format.id, wave_format.channel,  wave_format.sampling_rate,  wave_format.bits);
        wav_file.save_as(Path::new(&path_string)).unwrap();
        PathBuf::from(&path_string)
    }

    fn create_test_file_from_channel_vec(id: usize, channel: usize, sampling_rate: usize, bits: usize, channel_vec:&Vec<Vec<f64>>) -> PathBuf {
        let wave_format = WaveFormat {
            id: id,
            channel: channel,
            sampling_rate: sampling_rate,
    ...
    fn create_test_file_from_data_vec(id: usize, channel: usize, sampling_rate: usize, bits: usize, data_vec:&Vec<Vec<f64>>) -> PathBuf {
        let wave_format = WaveFormat {
            id: id,
            channel: channel,
            sampling_rate: sampling_rate,
    ...
    fn check_contents(file_path: &Path, vec_of_fmt_chunk: &[u8], vec_of_data_chunk: &[u8], vec_of_file: &[u8], expected_channel_vec:&Vec<Vec<f64>>) {
        {
            // file check
